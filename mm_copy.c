...
static struct block *extend_heap(size_t words) 
{
    ...
    struct block * ret = coalesce(blk);
    add_to_free_list(ret);
...
static struct block *find_fit(size_t asize)
{
    ...
    for (struct list_elem * current_list_elem = list_begin(&free_list);
        current_list_elem != list_end(&free_list);
        current_list_elem = list_next(current_list_elem)) {

        struct free_block * current_free_block = list_entry(current_list_elem, struct free_block, elem);
        struct block * current_block = (struct block*) current_free_block; 

        assert(blk_free(current_block));

        if (blk_size(current_block) >= asize) {
...
static void add_to_free_list(struct block *bp) {
    ...
    struct free_block * fbp = (struct free_block*) bp;
